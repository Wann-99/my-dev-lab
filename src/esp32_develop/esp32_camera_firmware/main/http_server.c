...
static esp_err_t status_handler(httpd_req_t *req) {
    ...
    *p++ = '{';
    ...
    p += sprintf(p, ",\"led_intensity\":%d", get_led_intensity());
    
    // Face detection placeholders (disabled for now)
...
static esp_err_t capture_handler(httpd_req_t *req) {
    ...
    ESP_LOGI(TAG, "JPG: %luB %lums", (unsigned long)(fb_len), (unsigned long)((fr_end - fr_start) / 1000));
    return res;
}
...
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    while(true){
        ...
        if (!fb) {
            ...
            // ESP_LOGI(TAG, "FB: %u bytes", fb->len);
            if(fb->format != PIXFORMAT_JPEG){
        ...
        if(res == ESP_OK){
            size_t hlen = snprintf((char *)part_buf, 128, _STREAM_PART, _jpg_buf_len, (long)_timestamp.tv_sec, (long)_timestamp.tv_usec);
