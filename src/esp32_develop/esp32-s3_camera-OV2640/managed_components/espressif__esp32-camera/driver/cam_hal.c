...
/* 首场首字节偶发为 0xFE（DVP 采样对齐），其后为 D8 FF 且能解出 JFIF，视作 SOI 并纠正 */
static inline void cam_try_fix_soi_leading_fe(uint8_t *buf, size_t len)
{
    if (len >= 3 && buf[0] == 0xFE && buf[1] == 0xD8 && buf[2] == 0xFF) {
        buf[0] = 0xFF;
    }
}

/* APP0：E0 00 10 J…；49/48 为 I/H，总线上 I 常被采成 H */
static bool cam_jpeg_app0_jfif_anchor(const uint8_t *b, size_t k, size_t scan)
{
    if (k + 6 >= scan) {
        return false;
    }
    return b[k] == 0xE0 && b[k + 1] == 0 && b[k + 2] == 0x10 && b[k + 3] == 0x4a && b[k + 4] == 0x46 &&
           (b[k + 5] == 0x49 || b[k + 5] == 0x48) && b[k + 6] == 0x46;
}

/* 用 APP0/JFIF 锚点对齐残缺 SOI（含 JFHF、FF 58 FF、ff ec d8 ff 等） */
static bool cam_try_recover_jpeg_soi_prefix(uint8_t *buf, size_t *plen, size_t fb_cap)
{
    size_t len = *plen;
    if (len < 16) {
        return false;
    }
    size_t scan = len < 256 ? len : 256;

    cam_try_fix_soi_leading_fe(buf, len);

    /* FE D8 FE E0…（两个 FF 都被采成 FE，串口常见） */
    if (len >= 8 && buf[0] == 0xFE && buf[1] == 0xD8 && buf[2] == 0xFE && buf[3] == 0xE0 && buf[4] == 0 &&
        buf[5] == 0x10 && buf[6] == 0x4a) {
        buf[0] = 0xFF;
        buf[2] = 0xFF;
        return true;
    }

    /* FF [杂] D8 FE|FF E0 00 10 4A…（如 ff e4 d8 fe e0，SOI 中间多一字节） */
    if (len >= 8 && buf[0] == 0xFF && buf[1] != 0xD8 && buf[2] == 0xD8 && buf[4] == 0xE0 && buf[5] == 0 &&
        buf[6] == 0x10 && buf[7] == 0x4a && (buf[3] == 0xFE || buf[3] == 0xFF)) {
        memmove(buf + 1, buf + 2, len - 2);
        len--;
        *plen = len;
        if (len >= 4 && buf[0] == 0xFF && buf[1] == 0xD8 && buf[2] == 0xFE && buf[3] == 0xE0) {
            buf[2] = 0xFF;
        }
        return true;
    }

    /* 首字节即 D8…（整颗 FF SOI 丢失，串口常见 d8 ff e0 00 10 4a…） */
    if (len >= 8 && buf[0] == 0xD8 && buf[1] == 0xFF && buf[2] == 0xE0 && buf[3] == 0 && buf[4] == 0x10 && buf[5] == 0x4a) {
        if (len + 1 <= fb_cap) {
    ...
    /* FF D8 FE E0（APP0 首字节错成 FE，对应串口 ff d8 fe e0…） */
    if (len >= 4 && buf[0] == 0xFF && buf[1] == 0xD8 && buf[2] == 0xFE && buf[3] == 0xE0) {
        buf[2] = 0xFF;
        return true;
    }

    /* FF xx FF E0 00 10 4A：第二字节常为 D8 被采成 58 等 */
    if (len >= 7 && buf[0] == 0xFF && buf[2] == 0xFF && buf[3] == 0xE0 && buf[4] == 0 && buf[5] == 0x10 && buf[6] == 0x4a) {
        if (buf[1] != 0xD8) {
            buf[1] = 0xD8;
        }
        return true;
    ...
    /* JFIF 第三字符纠正（解码器多数仍能认 APP0） */
    for (size_t k = 0; k + 6 < scan; k++) {
        if (buf[k] != 0xE0 || buf[k + 1] != 0 || buf[k + 2] != 0x10 || buf[k + 3] != 0x4a || buf[k + 4] != 0x46 ||
            buf[k + 6] != 0x46) {
            continue;
        }
        if (buf[k + 5] == 0x48) {
    ...
    for (size_t k = 0; k + 6 < scan; k++) {
        if (!cam_jpeg_app0_jfif_anchor(buf, k, scan)) {
            continue;
        }
        if (k < 2) {
            continue;
        ...
        /* FF D8 FE EC … E0（坏两字节，E0 在 k） */
        if (k >= 4 && buf[k - 4] == 0xFF && buf[k - 3] == 0xD8 && buf[k] == 0xE0) {
            buf[k - 2] = 0xFF;
            buf[k - 1] = 0xE0;
            if (k - 4 > 0) {
                memmove(buf, buf + (k - 4), len - (k - 4));
                len -= (k - 4);
                *plen = len;
            }
            return true;
        ...
        /* FF D8 ? E0 且 ?!=FF：常见 FE / EC（E0 在 k，k>=3） */
        if (k >= 3 && buf[k] == 0xE0 && buf[k - 3] == 0xFF && buf[k - 2] == 0xD8 && buf[k - 1] != 0xFF) {
            buf[k - 1] = 0xFF;
            if (k - 3 > 0) {
                memmove(buf, buf + (k - 3), len - (k - 3));
                len -= (k - 3);
                *plen = len;
            }
            return true;
        ...
        /* 已是 FF D8 FF E0… */
        if (k >= 3 && buf[k - 3] == 0xFF && buf[k - 2] == 0xD8 && buf[k - 1] == 0xFF) {
            if (k - 3 > 0) {
                memmove(buf, buf + (k - 3), len - (k - 3));
                len -= (k - 3);
                *plen = len;
        ...
        if (k >= 3 && buf[k - 3] == 0xFE && buf[k - 2] == 0xD8 && buf[k - 1] == 0xFF && (k - 3) == 0) {
            buf[0] = 0xFF;
            return true;
        }

        if (buf[k - 2] == 0xD8 && buf[k - 1] == 0xFF) {
            size_t d8pos = k - 2;
            if (d8pos >= 2 && buf[d8pos - 2] == 0xFF && buf[d8pos - 1] == 0xD8) {
                continue;
            }
            if (d8pos == 1 && buf[0] == 0xFF) {
                continue;
            }
            if (d8pos == 0 && (len + 1) <= fb_cap) {
                memmove(buf + 1, buf, len);
                buf[0] = 0xFF;
                len++;
                *plen = len;
            ...
            if (d8pos > 0 && (len - d8pos + 1) <= fb_cap) {
                memmove(buf, buf + d8pos, len - d8pos);
                len -= d8pos;
                memmove(buf + 1, buf, len);
                buf[0] = 0xFF;
                len++;
...
/* 最后一个 SOS（FF DA）位置，EOI 只应出现在其后的熵编码段 */
static int cam_jpeg_last_sos_off(const uint8_t *buf, size_t length)
{
    int last = -1;
    if (length < 2) {
        return -1;
...
/* 从后往前找 EOI；D9 前一字节常被采成 FE/FC/E4；优先在 SOS 之后、避免误匹配表/段前填充 */
static int cam_verify_jpeg_eoi_relaxed(uint8_t *buf, size_t length)
{
    int off = cam_verify_jpeg_eoi(buf, length, false);
    if (off >= 0) {
        return off;
    }
    if (length < 2) {
    ...
    int sos = cam_jpeg_last_sos_off(buf, length);
    int lo = (sos >= 0) ? (sos + 2) : 0;
    for (int i = (int)length - 2; i >= lo; i--) {
        if (buf[(size_t)i + 1] != 0xD9) {
            continue;
        }
        uint8_t b = buf[i];
        if (b == 0xFF) {
                ...
                if (cam_event == CAM_IN_SUC_EOF_EVENT) {
                    ...
                    if (cam_obj->jpeg_mode && cnt == 0) {
                        if (cam_obj->psram_mode) {
                            ...
                            cam_try_fix_soi_leading_fe(frame_buffer_event->buf, probe_len);
                            ...
                            (void)cam_try_recover_jpeg_soi_prefix(frame_buffer_event->buf, &frame_buffer_event->len,
                                                                  cam_obj->fb_size);
                            cam_try_fix_soi_leading_fe(frame_buffer_event->buf, frame_buffer_event->len);
                            int soi_off = cam_verify_jpeg_soi(frame_buffer_event->buf, frame_buffer_event->len);
                            if (soi_off < 0) {
                                static uint32_t soi_miss_diag = 0;
                                soi_miss_diag++;
                                if ((soi_miss_diag % 50u) == 1u) {
                                    size_t n = frame_buffer_event->len;
                                    if (n > 32) {
                                        n = 32;
                                    }
                                    ESP_LOGW(TAG, "SOI missing, 1st chunk %u B, miss #%lu, hex:",
                                             (unsigned)frame_buffer_event->len, (unsigned long)soi_miss_diag);
                                    ESP_LOG_BUFFER_HEXDUMP(TAG, frame_buffer_event->buf, n, ESP_LOG_WARN);
                                }
                            ...
                            if (soi_off > 0) {
                                size_t new_len = frame_buffer_event->len - (size_t)soi_off;
                                memmove(frame_buffer_event->buf,
                                        frame_buffer_event->buf + soi_off,
                                        new_len);
                                frame_buffer_event->len = new_len;
                            }
                        }
                    }
    ...
    for (;;)
    {
        ...
        if (cam_obj->jpeg_mode) {
            ...
            if (cam_obj->psram_mode) {
                ...
                /* 去掉 DVP/DMA 常见尾填 0，便于从真实末尾反找 EOI */
                while (dma_buffer->len > 64 &&
                       dma_buffer->buf[dma_buffer->len - 1] == 0) {
                    dma_buffer->len--;
                }
                offset_e = cam_verify_jpeg_eoi_relaxed(dma_buffer->buf, dma_buffer->len);
            }
...
skip_eoi_check:
            /* DVP 丢尾：补 FF D9；满缓冲则覆盖最后 2 字节 */
            if (!cam_obj->psram_mode && dma_buffer->len >= 4 && dma_buffer->buf[0] == 0xFF &&
                dma_buffer->buf[1] == 0xD8) {
                static uint16_t eoi_synth_cnt = 0;
                if (dma_buffer->len + JPEG_EOI_MARKER_LEN <= cam_obj->fb_size) {
                    size_t L = dma_buffer->len;
                    dma_buffer->buf[L] = JPEG_EOI_BYTES[0];
                    dma_buffer->buf[L + 1] = JPEG_EOI_BYTES[1];
                    dma_buffer->len = L + JPEG_EOI_MARKER_LEN;
                    CAM_WARN_THROTTLE(eoi_synth_cnt, "JPEG EOI synthesized (append)");
                    return dma_buffer;
                }
                if (dma_buffer->len >= JPEG_EOI_MARKER_LEN && dma_buffer->len == cam_obj->fb_size) {
                    dma_buffer->buf[dma_buffer->len - 2] = JPEG_EOI_BYTES[0];
                    dma_buffer->buf[dma_buffer->len - 1] = JPEG_EOI_BYTES[1];
                    CAM_WARN_THROTTLE(eoi_synth_cnt, "JPEG EOI synthesized (overwrite tail)");
                    return dma_buffer;
                }
            }
