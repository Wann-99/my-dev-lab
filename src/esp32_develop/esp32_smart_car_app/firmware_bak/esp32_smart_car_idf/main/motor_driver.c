...
// Motor Speed State
static int target_speeds[4] = {0, 0, 0, 0};
static int current_speeds[4] = {0, 0, 0, 0};
static int g_max_speed = 1023;
#define RAMP_STEP 50 // Increase/Decrease duty by 50 every 20ms (~0.4s for full range)
...
#define MIN_DUTY 550   // 稍微降低起始占空比，平衡起步力矩与低速平滑性

static void update_motor_hardware(int motor_id, int speed) {
    int in1_pin = -1;
    int in2_pin = -1;
    ...
    // 优化的死区映射：确保从 MIN_DUTY 平滑过渡到 MAX_SPEED
    if (abs_speed > 5) { 
        // 映射公式: MIN_DUTY + (abs_speed / 1023) * (1023 - MIN_DUTY)
    ...
    if (speed > 0) {
        ...
        // Electronic Brake: Short IN1 and IN2 to stop faster
        gpio_set_level(in1_pin, 1);
        gpio_set_level(in2_pin, 1);
...
static void motor_ramp_task(void *pvParameters) {
    while (1) {
        for (int i = 0; i < 4; i++) {
            if (current_speeds[i] < target_speeds[i]) {
                current_speeds[i] += RAMP_STEP;
                if (current_speeds[i] > target_speeds[i]) current_speeds[i] = target_speeds[i];
                update_motor_hardware(i + 1, current_speeds[i]);
...
void motor_set_speed(int motor_id, int speed) {
    ...
    target_speeds[motor_id - 1] = speed;
    
    // If target is 0 (stop), we might want to bypass ramp for safety, 
    // but usually ramping down is smoother. 
    // For emergency stop, use motor_stop().
}

void motor_init(void) {
    ESP_LOGI(TAG, "Initializing Motors with Ramping...");
    ...
    // After initialization, start the ramp task
    xTaskCreate(motor_ramp_task, "motor_ramp_task", 2048, NULL, 5, NULL);

    ESP_LOGI(TAG, "Motors initialized with ramp task");
}
...
void move_car(float vx, float vy, float vw) {
    ...
    ESP_LOGI(TAG, "Move Car: vx=%.2f, vy=%.2f, vw=%.2f (Max:%d) -> M1:%d, M2:%d, M3:%d, M4:%d", 
             vx, vy, vw, g_max_speed, m1, m2, m3, m4);
...
void motor_set_max_speed(int speed) {
    ...
    if (speed > 1023) speed = 1023;
    g_max_speed = speed;
    ESP_LOGI(TAG, "Global Max Speed set to: %d", g_max_speed);
}
