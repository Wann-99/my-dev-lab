...
class CarState extends ChangeNotifier {
  ...
  Future<void> downloadAndInstallApp() async {
    ...
    // iOS does not support sideloading APKs.
    // Open the TestFlight / App Store URL in the browser instead.
    _otaStatus = '正在打开更新页面...';
    _otaProgress = 0.0;
    notifyListeners();

    try {
      final uri = Uri.parse(_appDownloadUrl);
      if (await canLaunchUrl(uri)) {
        await launchUrl(uri, mode: LaunchMode.externalApplication);
        _otaStatus = '已在浏览器中打开更新页面';
      } else {
        _otaStatus = '无法打开链接，请手动访问 App Store / TestFlight';
      }
    } catch (e) {
      _otaStatus = '打开更新链接失败: $e';
    }
    _otaProgress = 1.0;
    notifyListeners();
  }
