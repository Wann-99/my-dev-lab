...
// ── Encoder-based in-place turn ──────────────────────────────────────────────
// Closed-loop turnaround: spin in place at a controlled speed and stop once the
// wheels have rolled the target number of encoder pulses (measured live in the
// control task). This is accurate regardless of battery level / floor friction,
// unlike a timed spin.
#define TURN_SPEED_CPI       120     // target counts per 50ms loop while turning (~0.5-0.6 max; balance speed vs overshoot)
#define TURN_COUNTS_PER_360  21000   // default avg pulses/wheel for a full 360° spin (tunable; app may override via "counts")
static volatile bool g_turn_active = false;
static int g_turn_dir = 1;
static int64_t g_turn_target = 0;          // target avg |delta| counts per wheel
static int64_t g_turn_base[4] = {0, 0, 0, 0};
...
static void motor_control_task(void *pvParameters) {
    ...
    while (1) {
        ...
        // ── Encoder-based in-place turn: drive spin targets, stop on count ──
        if (g_turn_active) {
            int64_t prog = (llabs(curr_count[0] - g_turn_base[0]) +
                            llabs(curr_count[1] - g_turn_base[1]) +
                            llabs(curr_count[2] - g_turn_base[2]) +
                            llabs(curr_count[3] - g_turn_base[3])) / 4;
            if (prog >= g_turn_target) {
                // Reached target angle — stop and exit turn mode.
                g_turn_active = false;
                target_speeds[0] = target_speeds[1] = target_speeds[2] = target_speeds[3] = 0;
                ESP_LOGI(TAG, "Turn complete: avg %lld / %lld counts", (long long)prog, (long long)g_turn_target);
            } else {
                // In-place rotation (mecanum vw): FL=+, FR=-, RL=+, RR=- .
                int s = TURN_SPEED_CPI * g_turn_dir;
                target_speeds[0] =  s;
                target_speeds[1] = -s;
                target_speeds[2] =  s;
                target_speeds[3] = -s;
...
void move_car(float vx, float vy, float vw) {
    // Any explicit move command cancels an in-progress encoder turn.
    g_turn_active = false;
...
// ── Encoder-based in-place turn API ──────────────────────────────────────────
void motor_turn_counts(long long counts, int dir) {
    if (counts <= 0) return;
    // Snapshot current encoder positions as the reference.
    g_turn_base[0] = encoder_get_count(&enc_m1);
    g_turn_base[1] = encoder_get_count(&enc_m2);
    g_turn_base[2] = encoder_get_count(&enc_m3);
    g_turn_base[3] = encoder_get_count(&enc_m4);
    g_turn_target = counts;
    g_turn_dir = (dir >= 0) ? 1 : -1;
    g_turn_active = true;
    ESP_LOGI(TAG, "Turn start: target %lld counts, dir %d", counts, g_turn_dir);
}

void motor_turn_angle(float angle_deg, int dir) {
    if (angle_deg < 0) { angle_deg = -angle_deg; dir = -dir; }
    long long counts = (long long)((double)TURN_COUNTS_PER_360 * (angle_deg / 360.0));
    motor_turn_counts(counts, dir);
}

bool motor_turn_active(void) { return g_turn_active; }
