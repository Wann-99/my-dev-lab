...
esp_err_t handle_car_command(const char *json_data, char *response_buffer, size_t response_len) {
    ...
    if (strcmp(cmd_str, "ping") == 0) {
        ...
        move_car(vx, vy, vw);
    } else if (strcmp(cmd_str, "turn") == 0) {
        // Encoder-based in-place turn (closed-loop). Manual override.
        if (g_car_mode != 0) g_car_mode = 0;
        int dir = 1;
        cJSON *j_dir = cJSON_GetObjectItem(root, "dir");
        cJSON *j_counts = cJSON_GetObjectItem(root, "counts");
        cJSON *j_angle = cJSON_GetObjectItem(root, "angle");
        if (j_dir) dir = j_dir->valueint;
        if (j_counts) {
            // Direct pulse target — lets the app calibrate without reflashing.
            motor_turn_counts((long long)j_counts->valuedouble, dir);
        } else {
            float angle = j_angle ? (float)j_angle->valuedouble : 180.0f;
            motor_turn_angle(angle, dir);
        }
        ESP_LOGI(TAG, "CMD: turn dir=%d", dir);
        if (response_buffer) snprintf(response_buffer, response_len, "{\"res\":\"ok\",\"msg\":\"turning\"}");
