...
class NodeVisitor(ast.NodeVisitor):
    ...
    def visit_Assign(self, node):
        ...
        if (
            ...
            # 函数内部对类变量赋值
            if hasattr(node.targets[0], "value") and node.targets[0].value.id == "self":
    ...
    def visit_Attribute(self, node):
        """Visit attribute"""
        line = "{object}.{attr}"
        if hasattr(node, "value") and node.value.id == "self" and node.attr.startswith("pp_"):
            line = "{attr}"
            self.emit(line.format(attr=node.attr[3:]))
        else:
            values = {
    ...
    def visit_Call(self, node):
            ...
            if isinstance(arg, ast.Attribute):
                ...
                for el in arg.elts:
                    if isinstance(el, ast.Attribute):
                        el = get_full_attribute(el)
                        enum_class_name, member_name = el.split(".")
                        enum_class = globals().get(enum_class_name)
                        if enum_class and hasattr(enum_class, member_name):
                            enum_member = getattr(
                                enum_class, member_name
                            ...
                            real_value = getattr(enum_member, "value", None)
                            arg_list.append(ast.Constant(value=str(real_value)))
                        else:
                            arg_list.append(el)
                    else:
                        arg_list.append(el)
                a = ast.List(elts=arg_list, ctx=arg.ctx)
                arguments.append(self.visit_all(a, inline=True))
            else:
                arguments.append(self.visit_all(arg, inline=True))
        if name == "print":
            name = "info"
        ...
        elif name in ["set_global_var", "get_global_var"]:
            line = line.format(name=name, arguments=", ".join(arguments))
            self.called_args["variable"].append(node.args[0].value)
        elif name in ["set_tool", "get_tool"]:
            line = line.format(name=name, arguments=", ".join(arguments))
            self.called_args["tool"].append(node.args[0].value)
        elif name in ["set_workcoord", "get_workcoord"]:
            line = line.format(name=name, arguments=", ".join(arguments))
            self.called_args["workcoord"].append(node.args[0].value)
        else:
            line = line.format(name=name, arguments=", ".join(arguments))
        self.emit(line)
    ...
    def visit_str(self, node):
        """
        visit str like "self.var"
        :param node:
        :return:
        """
        if node.startswith("self.") and node.count(".") == 1:
            self.emit(node[8:])
    ...
    def visit_all(self, nodes, inline=False):
        ...
        if isinstance(nodes, list):
            ...
            if not inline:
                ...
                self.called_args["variable"].extend(visitor.called_args["variable"])
                self.called_args["tool"].extend(visitor.called_args["tool"])
                self.called_args["workcoord"].extend(visitor.called_args["workcoord"])
        else:
            visitor.visit(nodes)
            if not inline:
